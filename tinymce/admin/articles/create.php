...
try {
    $categories = fetch_available_categories();
} catch (Throwable $e) {
    $error = 'Erreur chargement categories: ' . $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $selectedCategoryIds = isset($_POST['category_ids']) && is_array($_POST['category_ids'])
                ...
                if (isset($_FILES['extra_images']['name']) && is_array($_FILES['extra_images']['name'])) {
                    ...
                    try {
                        foreach ($_FILES['extra_images']['name'] as $idx => $unusedName) {
                            ...
                            if ((int)$single['error'] === UPLOAD_ERR_NO_FILE) continue;
    ...
    if ($isEdit) {
        ...
        try {
            ...
            if ($error === '') $error = 'Erreur serveur: ' . $e->getMessage();
        }
    }
}
      ...
      <div class="form-grid">
        ...
        <?php if (!$isEdit): ?>
          <div class="full">
            ...
            <?php if (count($categories) === 0): ?>
              <div class="muted">Aucune catégorie disponible. Créez d'abord une catégorie dans Gestion catégorie.</div>
            <?php else: ?>
              <div class="category-check-grid">
                <?php foreach ($categories as $category): ?>
                  <?php $categoryId = (int)($category['Id_categorie'] ?? 0); ?>
        ...
        <div class="full">
          ...
          <div class="muted" style="margin-bottom:8px;">
            Utilisez le bouton Image de TinyMCE pour téléverser vers la base média.
          </div>
        ...
        <?php if ($isEdit): ?>
          <div class="full">
            ...
            <div class="muted" style="margin-top:6px;">
              Les nouvelles photos seront enregistrées dans /uploads/articles/modif/article-<?= (int)$articleId ?>.
            </div>
          </div>
